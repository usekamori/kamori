...
> **🔒 Authentication defaults.** Kamori self-hosted is single-tenant — one `INGEST_TOKEN` secures the whole instance (there are no projects or per-key scopes; that's the Cloud edition).
> - **`npx kamori`** generates a strong token for you by default (written to `.env`, shown in the setup output). Opt out with `--no-token`.
> - The **Docker image** generates and persists a token on first run if you don't provide one (printed once to the container logs). Opt out with `-e KAMORI_ALLOW_NO_AUTH=true`.
> - If the server runs with an empty token it is **open** (anyone who can reach it can read, write, and delete logs). It logs a warning, and **refuses to start** when bound to a network interface with `NODE_ENV=production`. Set `INGEST_TOKEN` before exposing it to any network.
...
Omit `-e INGEST_TOKEN` and the container generates one on first run, persists it to the `kamori-data` volume, and prints it to the logs (`docker logs kamori`). Pass `-e KAMORI_ALLOW_NO_AUTH=true` to run without auth on a trusted network.
...
Options:
  ...
  --log-token <secret>    Set INGEST_TOKEN explicitly (otherwise one is generated).
  --mcp-token <secret>    Set MCP_TOKEN explicitly for MCP HTTP Bearer auth (@usekamori/mcp).
  --no-token              Run the ingest API without auth (empty INGEST_TOKEN).
  ...
  --yes, -y               Non-interactive — skip prompts (Node path + MCP on unless --no-mcp).
                          A strong INGEST_TOKEN (and MCP_TOKEN) is generated by default;
                          use --no-token / --no-mcp-token to run open. Also triggered when
                          CI=true or stdin is not a TTY.
```
...
# Combine with service/level filters (level = exact match)
curl -sS "http://localhost:3110/v1/search?q=connection+refused&service=api&level=error" \
  -H "Authorization: Bearer your-secret" | jq
...
curl -sS "http://localhost:3110/v1/count?service=api&level=error&since=2026-07-01T00:00:00Z" \
  -H "Authorization: Bearer your-secret"

# Hourly histogram for charts
curl -sS "http://localhost:3110/v1/histogram?bucket=1h&level=error" \
  -H "Authorization: Bearer your-secret"
