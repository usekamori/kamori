...
async function main(): Promise<void> {
  ...
  // Undefined choice ⇒ generate a token by default (secure out of the box).
  let logChoice: TokenChoice | undefined;
  let mcpChoice: TokenChoice | undefined;

  /** Prompt for a token: generate (default), disable, or enter your own. */
  async function promptTokenChoice(
    rl: readline.Interface,
    name: string,
  ): Promise<TokenChoice> {
    const gen = (await prompt(rl, `Generate a secure ${name}? (Y/n): `))
      .trim()
      .toLowerCase();
    if (gen === "n" || gen === "no") {
      const own = (
        await prompt(rl, `Enter your own ${name} (leave blank to disable auth): `)
      ).trim();
      return own === "" ? "disable" : { value: own };
    }
    return "generate";
  }
  ...
  if (interactive) {
    ...
    try {
      ...
      if (flags.logToken === undefined && !flags.noToken) {
        logChoice = await promptTokenChoice(rl, "INGEST_TOKEN");
      }
      ...
      if (mcp && flags.mcpToken === undefined && !flags.noMcpToken) {
        mcpChoice = await promptTokenChoice(rl, "MCP_TOKEN");
      }
    ...
    // logChoice / mcpChoice stay undefined → a token is generated by default
    // (unless --no-token / --log-token override).
  }

  const logToken = resolveLogToken(flags, logChoice);
  const mcpToken = resolveMcpToken(mcp, flags, mcpChoice);
